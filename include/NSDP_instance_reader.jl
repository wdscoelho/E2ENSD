...
function get_Instance(input_folder::String,mp::String)
   ...
   #Here, we get all proprities needed to represent the instance
        ...
        instance = Instance(set,
                            get_physical_network(joinpath(input_folder,"my_phyNet.dat")),
                            get_NS_requests(joinpath(input_folder,"instance_$(mp).dat")))
        ...
        instance.number_of_NFs = cnf*length(instance.setSlices) + anf*number_commodities#this is tha max number of NFs 
        instance.slice_cont = get_slice_cont(joinpath(input_folder,"slice_cont_1.dat"))
        ...
        for ln in eachline(file)
            ...
            elseif aux_string[1]  == "slice_request" 
                ...
                push!(setSlices, SliceRequest(parse(Int8,aux_string[4]),parse(Int16,aux_string[10]),
                                        ...
                                        aux_set_VNFs_to_install,get_commodities(Meta.parse(aux_string[22])), parse(Float16,aux_string[24]),get_node_sharing(Meta.parse(aux_string[26]))))
...
function get_slice_cont(file::String)
    #auxiliar variables
    number_of_slices = 0
